...
class MilkManufactureInfo extends StatefulWidget {
  static const routeName = 'milk_manufacturing_info';

  _HomePageState createState() => _HomePageState();
}

class _HomePageState extends State<MilkManufactureInfo> {
  List<TextEditingController> _controllers = [];
  List<String> _cattleGender = ['সকাল', 'সন্ধ্যা'];
  String? selectedFarms, selectedTime;
    ...
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(
          ...
          centerTitle: true,
          title: Text('দুধ উৎপাদন সম্পর্কিত তথ্য'),
        ),
                    ...
                    child: AbsorbPointer(
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: 'দুধ দহন করার তারিখ (dd-mm-yyyy)',
                        ),
                      ),
                    )),
              ),
                          ...
                          Column(
                            ...
                            children: <Widget>[
                              ...
                              Container(
                                  ...
                                  alignment: Alignment.centerLeft,
                                  child: Text(
                                    'ট্যাগ: ${cattleFarmsList[index].tag}',
                                    style: TextStyle(
                                      fontSize: 14,
                                      color: Colors.grey,
                                    ),
  ...
  void setCattleData() {
    if (selectedFarms == null) {
      ...
      Common.setWrongToast('দুধ দহন করার তারিখ প্রদান করুন');
    } else if (selectedTime == null) {
      Common.setWrongToast('দহনের সময় নির্বাচন করুন!');
    } else {
  ...
  void dataPush() async {
    ...
    for (int i = 0; i < cattleFarmsList.length; i++) {
      ...
      pro = [Production(cattleId: id, value: value)];
    }
    int farm = int.parse(selectedFarms!);
    assert(farm is int);

    var body = {"farm_id": selectedFarms, "date": _date.text, "time": selectedTime, "productions": pro.toString()};
    print(body);

    /*var body = {"farm_id": selectedFarms, "date": _date.text, "time": selectedTime, "productions": pro.toString()};
    print(body);
    http.Response response =
        await http.post(Uri.parse(NetworkServices.baseUrl + 'form/store-milk-production'),
            body: body, headers: headersToken);*/

    // print(pro.toString());
    http.Response response =
    await http.post(Uri.parse(NetworkServices.baseUrl + 'form/store-milk-production',), body: {
      "farm_id": selectedFarms,
      "date": _date.text,
      "time": selectedTime,
      "productions": json.encode(pro),
  ...
  void dataInsertSuccess(BuildContext context) async {
    Alert(
      ...
      closeFunction: () {
        Navigator.of(context).pushReplacementNamed(CowRegistrationHome.routeName);
      },
      title: '',
