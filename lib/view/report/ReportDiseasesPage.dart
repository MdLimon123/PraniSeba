...
import 'package:prani_sheba/controller/ReportDiseaseController.dart';
...
class ReportDiseasesPage extends StatefulWidget {
  static const routeName = 'report_diseases_page';

  _HomePageState createState() => _HomePageState();
}

class _HomePageState extends State<ReportDiseasesPage> {
  final GlobalKey<ScaffoldState> _key = GlobalKey<ScaffoldState>();

  final ReportDiseaseController productController = Get.put(ReportDiseaseController());
  ...
  CattleDiseases? cattleDiseases;

  Future<List<CattleDiseases>> _getDiseaseHistories() async {
    List<CattleDiseases> cattle = [];
    for (int i = 0; i < cattle_diseasesToList!.length; i++) {
      cattleDiseases = CattleDiseases();
      cattleDiseases!.id = cattle_diseasesToList![i].id;
      cattleDiseases!.name = cattle_diseasesToList![i].name;
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                alignment: Alignment.centerLeft,
                child: Text('রোগ', textAlign: TextAlign.start, style: TextStyle(color: Colors.blueGrey, fontSize: 16)),
              ),
              size15,
              Row(
                children: [
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Container(
                                        margin: EdgeInsets.only(top: 5),
                                        child: Text(
                                          'আক্রান্ত সময়ঃ ${DateConverter.formatDateIOS(productController.dataList[index].date!)}',
                                          textAlign: TextAlign.left,
                                          maxLines: 1,
                                          overflow: TextOverflow.ellipsis,
                                          style: TextStyle(fontSize: 15, color: Colors.grey, fontStyle: FontStyle.normal),
  ...
  void setCattleData() {
    if (selectedFarms == null) {
      ...
      productController.fetchReportDisease(
          selectedFarms!, selectedCattleGroupId!, selectedCattleId!, _dateBirthDay, _dateLastDay, diseaseHistoryChecked.toList().toString());
    }
  }
}
